...
bool Agent::returnHomeRequired(const Perception& p, Direction* direction) {
    ...
    if (battery_->Level() < min_battery_level) {
      std::cout << "Heading home!" << std::endl; // @TODO: remove this debug output

      // Simple homing strategy:
      //   * If the distance to the station has decreased since the last step, go to a new obstacle free direction.
      ...
      if(current_charging_signal > last_charging_signal_ && last_charging_signal_ >= 0)
      {
